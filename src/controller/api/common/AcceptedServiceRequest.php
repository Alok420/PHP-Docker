...
//SELECT * FROM `service_requests` WHERE `userdate` >= '2020-09-04' AND `userdate` < '2020-09-08' AND`status` LIKE 'pending'
    ...
    if($status=='accepted') {
        $status = "Accepted' OR `status` LIKE 'Verified"; 
    }

    $query = "select * from $tbname WHERE `userdate` >= '$fromdate' AND `userdate` < '$todate' AND `status` LIKE '$status' AND `vendorid` LIKE '$loginid'";

    if($todate == ''){
        $query = "select * from $tbname WHERE `status` LIKE '$status' AND `vendorid` LIKE '$loginid'";
    } 
    else if($todate == $fromdate) {
        $query = "select * from $tbname WHERE `userdate` LIKE '$todate' AND `status` LIKE '$status' AND `vendorid` LIKE '$loginid'";
    }
...
while ($row = $result->fetch_assoc()) {
    ...
    $data[$i]=array("service"=>$row, "vehicle"=>$vehicle, "service_type"=>$service_type, "vehicle_model"=>$vehicle_model,"vehicle_make"=>$vehicle_make,"vehicle_type"=>$vehicle_type, "user"=> $user);
    $i++;
