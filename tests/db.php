...
class DB implements DBDeclare {
    ...
    public function promocode($name) {
        $twoDigName = strtoupper(substr($name, 0, 2));
        $rand = rand(0, 1000);
        $promocode = "PR".$twoDigName . $rand;
        return $promocode;
    }
    
    public function business_promocode($name) {
        $twoDigName = strtoupper(substr($name, 0, 2));
        $rand = rand(0, 1000);
        $business_promocode = "PRBS".$twoDigName . $rand;
        return $business_promocode;
    }
        ...
        <h1 style="text-align: center;">
            <?php
                    if (isset($row["image"]) && $row["image"] != "") {
                        ?>
            <img style="margin: auto;" src="../img/<?php echo $table . "/" . $row["image"]; ?>"
...
//                            if table has name with underscore-------------------
                        ...
                        if (is_array($columns)) {
                            ...
                            if (is_array($columnlavel2)) {
                                ...
                                echo '<tr><th>' . $keymask . '</th><td>' . $val . '</td><tr>';
                            } else if ($key == "id") {
...
//            this loop is for table heading --------------------------------------------
            for ($i = 0; $i < count($columns); $i++) {
                ...
                if ($columns[$i] == "id") {
                    echo "<th></i><a href='#' style='text-decoration:none;color:white;'>" . ucwords(str_replace("_", "&nbsp;", $columns[$i])) . "</a></th>";
                } else {
...
//            end of table heading---------------------------
        ...
        <form method="POST" action="../controller/update.php" enctype="multipart/form-data"
            ...
            <td><button type="button" class='btn btn-outline-success btn-sm' data-toggle='modal'
                    ...
                    onclick='updateRecord("<?php echo $row["id"]; ?>", "<?php echo $column; ?>", "<?php echo $table; ?>")'
                    id='updatebtn'><i class="fas fa-edit"></i></button></td>
            <?php
                    }
                    ...
                    ?>
            <td><a type="button" class='btn btn-outline-success btn-sm' onclick="return confirm('Delete this record?')"
                    href="../controller/DeleteController.php?id=<?php echo $row['id'] ?>&table_name=<?php echo $table ?>"><i
...
//          here we set the display name instead of showing id of referenced table-----------------------
                foreach ($row as $key => $val) {
                    ...
                    if (strpos($key, "_id") > 0) {
                        ...
                        } else if ($tb_key[0] == "books") {
                            $users = $this->select("books", "id,name", array("id" => $val));
                            $user = $users->fetch_assoc();
                            $name_id = $user["name"];
                        } else if ($tb_key[0] == "member_reward_chart") {
                            
                            $users = $this->select("member_reward_chart", "id,reward", array("id" => $val));
                            $user = $users->fetch_assoc();
                            $name_id = $user["reward"];
                        }else if ($tb_key[0] == "reward_chart") {
                            $users = $this->select("reward_chart", "id,reward", array("id" => $val));
                            $user = $users->fetch_assoc();
                            $name_id = $user["reward"];
                        } else if ($tb_key[0] == "ad" && $tb_key[1] == "places") {
                            $users = $this->select("ad_places", "id,name", array("id" => $val));
...
//--------------------end of displaying name instead of showing id------------------------------------------------
                        ...
                        echo '<td class="' . $key . '"><div style="width: 302px;overflow: hidden;">';
                        if ($ext == "jpg" || $ext == "png" || $ext == "gif" || $ext == "JPG" || $ext == "PNG" || $ext == "jpeg" || $ext == "GIF") {
                            echo '<a target="_blank" href="' . $filepath .'/'. $val . '"><img style = " height:35px; width:auto;" src="' . $filepath .'/'. $val . '"> ' . $val . '</a>';
                        } else {
...
//-------------------------this function is not tested yet-------------
        ...
        <?php
            ...
            while ($row = $list->fetch_assoc()) {
                ...
                if ($toollist == "update") {
                    ?>
        <td><button class='btn btn-outline-success btn-sm' data-toggle='modal' data-target='#updatemodel'
                onclick='updateRecord("<?php echo $row["id"]; ?>", "<?php echo $column; ?>", "<?php echo $table; ?>")'
                id='updatebtn'><i class="fas fa-edit"></i></button></td>
        <?php
                } else if ($toollist == "delete") {
                    ?>
        <td><button class='btn btn-outline-success btn-sm' data-toggle='modal' data-target='#deletemodel'
                onclick='deleteRecord("<?php echo $row["id"]; ?>", "<?php echo $table; ?>")' id='deletebtn'><i
